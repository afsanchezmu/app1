...
<script type="text/javascript">if (typeof wr !== "undefined") { wr.ui.html.resx.refreshLoaded(); }</script>
  ...
  <body>
	...
	<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
		<div id="_wr_page" title="app1;mov1#mpkg1#mpkg2#hym1#page13"></div>
		...
		<c:if test="${wrBoxed}">
			...
			<html:hidden property="lastURL" styleId="lastURL_page13"/>
				...
				<input type="hidden" name="ln79" value="<webratio:Link link="ln79"/>">
				<input type="hidden" name="ln80" value="<webratio:Link link="ln80"/>">
				<input type="hidden" name="ln82" value="<webratio:Link link="ln82"/>">
	    <div id="top">
	    	<div class="user">
	    		<div class="container">
		...
		<c:if test="${not (empty btFieldError)}">
			<div class="alert alert-danger alert-dismissable">
				...
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					...
					<html:text   styleId="fld11" styleClass="  form-control " property="fld11" readonly="false"/>
...
<div class="EntryUnit ">
	...
	<% pageContext.setAttribute("_wr_message", org.apache.commons.lang.StringEscapeUtils.escapeJavaScript((String) pageContext.getAttribute("_wr_message")));%>
		<c:if test="${webratio:isTargetAccessible('ln78', pageContext)}">
			<button title="Finish" onclick=" var c = confirm('<c:out value="${_wr_message}"/>');  if (c) { $('#page13FormBean')[0].target='_self' }; return c; " class="btn  btn-default " id="button:ln78" name="button:ln78" type="submit" value="Finish">
			Finish</button>
	</c:if>
			</c:if>
			<c:if test="${webratio:isTargetAccessible('ln79', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln79', pageContext)}">
			<button title="Save and Work Later" onclick="$('#page13FormBean')[0].target='_self'" class="btn  btn-default " id="button:ln79" name="button:ln79" type="submit" value="Save and Work Later">
			Save and Work Later</button>
	</c:if>
			</c:if>
			<c:if test="${webratio:isTargetAccessible('ln80', pageContext)}">
					 <c:if test="${webratio:isTargetAccessible('ln80', pageContext)}">
			<button title="Cancel" onclick="$('#page13FormBean')[0].target='_self'" class="btn  btn-default " id="button:ln80" name="button:ln80" type="submit" value="Cancel">
			Cancel</button>
	</c:if>
			</c:if>
		</div>
