...
<body style="background-color:white">
	...
	<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
		<div id="_wr_page" title="app1;sv2#page6"></div>
<div class="container_12">
				<div class="grid_12 alpha omega agrd_24">
<c:if test="${not(empty pdu1) and (pdu1.dataSize gt 0)}">
    <div class="diagram-title" style="width:${pdu1.width}px">
      <c:out value="${pdu1.processName}"/>
	</div>
	<div class="plain ">
		<div class="ProcessDiagramUnit" style="position:relative; width:${pdu1.width}px; height:${pdu1.height}px; z-index:1">
          <c:if test="${not(empty pdu1.data)}">
          <c:forEach var="current" varStatus="status" items="${pdu1.data}">
			   ...
			   <c:choose>
                  <c:when test="${not(empty current.parentProcessInstanceOid)}">
                      <tr>
                        ...
                        <td nowrap="nowrap"><a class="diagram-link" href="<webratio:Link escapeXml="true" link="pdu1parentLevel" position="index"/>" ><c:out value="${current.parentProcessInstanceName}"/></a></td>
                      </tr>
                  </c:when>
	              <c:when test="${not(empty current.parentProcesses)}">
	                  <c:forEach var="process" varStatus="status2" items="${current.parentProcesses}">
		                ...
		                <tr>
		                  <td nowrap="nowrap"><img src="Resources/parent_level.png"/></td>
		                  <td nowrap="nowrap"><a class="diagram-link" href="<webratio:Link escapeXml="true" link="pdu1parentLevel" position="index,index2"/>" ><c:out value="${process.model.name}"/></a></td>
		                </tr>
		              </c:forEach>
	              </c:when> 
	           </c:choose>
			   <c:choose>
			         ...
			         <c:forEach var="oid" varStatus="status2" items="${current.subProcessInstanceOids}">
		               ...
		               <tr>
		                 <td nowrap="nowrap"><img src="Resources/sub_level.png"/></td>
		                  <td nowrap="nowrap"><a class="diagram-link" href="<webratio:Link escapeXml="true" link="pdu1subLevel" position="index,index2"/>" ><c:out value="${current.subProcessInstanceNames[index2]}"/></a></td>
		               </tr>
		             </c:forEach>   
			      </c:when>
	              <c:when test="${not(empty current.subProcesses)}">
	                 <c:forEach var="process" varStatus="status2" items="${current.subProcesses}">
		               ...
		               <tr>
		                 <td nowrap="nowrap"><img src="Resources/sub_level.png"/></td>
		                 <td nowrap="nowrap"><a class="diagram-link" href="<webratio:Link escapeXml="true" link="pdu1subLevel" position="index,index2"/>" ><c:out value="${process.model.name}"/></a></td>
		               </tr>
		             </c:forEach> 
	              </c:when> 
	           </c:choose>
          ...
          <img src="<webratio:BLOB value="${pdu1.diagramFile}"/>" usemap="#diagramMap" width="${pdu1.width}" height="${pdu1.height}" border="0"/>
          <map name="diagramMap">
            <c:forEach var="current" varStatus="status" items="${pdu1.data}">
			  <c:set var="index" value="${status.index}"/>
    ...
    <div class="diagram-bottom" style="width:${pdu1.width}px"></div>
</c:if>
				</div>
					<div class="clear"></div>
</div>
