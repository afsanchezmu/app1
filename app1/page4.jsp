...
<script type="text/javascript">if (typeof wr !== "undefined") { wr.ui.html.resx.refreshLoaded(); }</script>
  ...
  <body>
	...
	<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
		<div id="_wr_page" title="app1;sv2#page4"></div>
			<html:form action="form_page4.do${wrAjax ? '#!ajax=true' : ''}" styleId="page4FormBean" enctype="multipart/form-data">
		<c:if test="${wrBoxed}">
			<script>
				jQuery("#page4FormBean").prop("action", "${wrBaseURI}form_page4.do${wrAjax ? '#!ajax=true' : ''}");
			</script>
		</c:if>
...
<c:if test="${not(empty pu9.exception)}">
...
<div class="plain ">
 <div class="plain ProcessUnit">
	<c:set var="exc" value="${pu9.exception}"/>
	<c:set var="errorTitle">
		<c:choose>
			<c:when test="${pu9.errorType eq 'ASSIGNED_ACTIVITY'}">Unable to start or resume the work item</c:when>
			<c:when test="${pu9.errorType eq 'COMPLETED_ACTIVITY'}">The work item has already been completed</c:when>
			<c:when test="${pu9.errorType eq 'ABORTED_ACTIVITY'}">The work item has been aborted</c:when>
	...
	<table>
		<c:choose>
		  <c:when test="${pu9.errorType eq 'ASSIGNED_ACTIVITY'}">
			<tr>
			  <td class="value">The work item "<b><c:out value="${pu9.activityInstanceName} [#${pu9.activityInstanceOid}]"/></b>" has already been assigned to another user and group.</td>
		    </tr>
		    <tr>
		       <td class="value" title="Assigned Group" style="padding-left:10px">
		       	Group: <b><c:out value="${pu9.activityGroupName} [#${pu9.activityGroupOid}]"/></b>
		       </td>
		    </tr>
		    <tr>
		       <td class="value" title="Assigned User" style="padding-left:10px">
		         User: <b><c:out value="${pu9.activityUserName} [#${pu9.activityUserOid}]"/></b>
		       </td>
		    </tr>
		    <tr>
		    	<td class="value">
		  ...
		  <c:when test="${pu9.errorType eq 'COMPLETED_ACTIVITY'}">
			<tr>
			  <td class="value">The work item "<b><c:out value="${pu9.activityInstanceName} [#${pu9.activityInstanceOid}]"/></b>" has already been completed.</td>
		    </tr>
		  ...
		  <c:when test="${pu9.errorType eq 'ABORTED_ACTIVITY'}">
			<tr><td class="value">The work item "<b><c:out value="${pu9.activityInstanceName} [#${pu9.activityInstanceOid}]"/></b>" has been aborted.</td></tr>
			<tr><td class="value">Go back to your <a class="link" href="<webratio:Link escapeXml="true" link="page5.link" position="index"/>">home</a> page and select another work item to execute.</td></tr>
		  </c:when>
		  <c:otherwise>
		    <tr>
