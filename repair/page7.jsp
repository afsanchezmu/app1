...
<html>
<head>
  <title>Process Instance Diagram</title>
...
<div class="container_12">
	...
	<div class="wr-ajaxDiv" id="pdu2_0">
...
<tr><td nowrap="nowrap"> <c:if test="${webratio:evaluateCondition('cexpr_var7', 'index', pageContext)}"><c:set var="_wr_message"><bean:message key="confirmRollback"/></c:set>
	<% pageContext.setAttribute("_wr_message", org.apache.commons.lang.StringEscapeUtils.escapeJavaScript((String) pageContext.getAttribute("_wr_message")));%>
		 			<a title="rollback" id="ln36" class=" diagram-link" href="<webratio:Link link="ln36" position="index"/>" onclick="if (confirm('<c:out value="${_wr_message}"/>')) { return true; } else { event.preventDefault ? event.preventDefault() : event.returnValue = false; return false; }">
					rollback</a></c:if>
 </td></tr> 
</c:if>
			   <c:choose>
