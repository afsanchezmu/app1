...
<PageRefresh>
		<LinkData>
			<![CDATA[<webratio:LinkData page="page14"/>]]>
		</LinkData>
		<c:if test="${webratio:isLayoutRefreshed('page14_grid_0', pageContext)}">
			<ControlData control="page14_grid_0" type="text/html">
				<![CDATA[<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
<div class="container_12">
				<div class="grid_12 alpha omega agrd_24">
					<div class="wr-ajaxDiv" id="page14_cell_0"></div>
				</div>
					<div class="clear"></div>
</div>
		...
		<c:if test="${webratio:isLayoutRefreshed('page14_cell_0,page14_grid_0', pageContext)}">
			<ControlData control="page14_cell_0" type="text/html">
				<![CDATA[<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
	<div class="wr-ajaxDiv" id="pdu4_0"></div>
		...
		<c:if test="${webratio:isLayoutRefreshed('pdu4_0,page14_cell_0,page14_grid_0', pageContext)}">
			   ...
			   <c:choose>
                  <c:when test="${not(empty current.parentProcessInstanceOid)}">
                      <tr>
                        ...
                        <td nowrap="nowrap"><a class="diagram-link" href="<webratio:Link escapeXml="true" link="pdu4parentLevel" position="index"/>" onclick="return ajaxRequest('<webratio:Link link="pdu4parentLevel" position="index" selectiveRefresh="true"/>', $H({pressedLink: 'pdu4parentLevel', sourcePage: 'page14'}))"><c:out value="${current.parentProcessInstanceName}"/></a></td>
                      </tr>
                  </c:when>
	              <c:when test="${not(empty current.parentProcesses)}">
	                  <c:forEach var="process" varStatus="status2" items="${current.parentProcesses}">
		                ...
		                <tr>
		                  <td nowrap="nowrap"><img src="Resources/parent_level.png"/></td>
		                  <td nowrap="nowrap"><a class="diagram-link" href="<webratio:Link escapeXml="true" link="pdu4parentLevel" position="index,index2"/>" onclick="return ajaxRequest('<webratio:Link link="pdu4parentLevel" position="index,index2" selectiveRefresh="true"/>', $H({pressedLink: 'pdu4parentLevel', sourcePage: 'page14'}))"><c:out value="${process.model.name}"/></a></td>
		                </tr>
		              </c:forEach>
	              </c:when> 
	           </c:choose>
			   <c:choose>
			         ...
			         <c:forEach var="oid" varStatus="status2" items="${current.subProcessInstanceOids}">
		               ...
		               <tr>
		                 <td nowrap="nowrap"><img src="Resources/sub_level.png"/></td>
		                  <td nowrap="nowrap"><a class="diagram-link" href="<webratio:Link escapeXml="true" link="pdu4subLevel" position="index,index2"/>" onclick="return ajaxRequest('<webratio:Link link="pdu4subLevel" position="index,index2" selectiveRefresh="true"/>', $H({pressedLink: 'pdu4subLevel', sourcePage: 'page14'}))"><c:out value="${current.subProcessInstanceNames[index2]}"/></a></td>
		               </tr>
		             </c:forEach>   
			      </c:when>
	              <c:when test="${not(empty current.subProcesses)}">
	                 <c:forEach var="process" varStatus="status2" items="${current.subProcesses}">
		               ...
		               <tr>
		                 <td nowrap="nowrap"><img src="Resources/sub_level.png"/></td>
		                 <td nowrap="nowrap"><a class="diagram-link" href="<webratio:Link escapeXml="true" link="pdu4subLevel" position="index,index2"/>" onclick="return ajaxRequest('<webratio:Link link="pdu4subLevel" position="index,index2" selectiveRefresh="true"/>', $H({pressedLink: 'pdu4subLevel', sourcePage: 'page14'}))"><c:out value="${process.model.name}"/></a></td>
		               </tr>
		             </c:forEach> 
	              </c:when> 
	           </c:choose>
