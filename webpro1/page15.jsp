...
<html>
  <head>
    ...
    <title>Definicion de la idea</title>
	...
	<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
		...
		<c:if test="${wrBoxed}">
			...
			<html:hidden property="lastURL" styleId="lastURL_page15"/>
				<input type="hidden" name="ln104" value="<webratio:Link link="ln104"/>">
				<input type="hidden" name="ln106" value="<webratio:Link link="ln106"/>">
				<input type="hidden" name="ln107" value="<webratio:Link link="ln107"/>">
				<input type="hidden" name="ln108" value="<webratio:Link link="ln108"/>">
	    <div id="top">
	    	<div class="user">
	    		<div class="container">
	    		</div>
	    ...
	    <div class="container">
			<div id="breadcrumb">
				...
				<span>Definicion de la idea</span>
			</div>
			<div id="page-links">
			</div>
		</div>
...
<div class="panel panel-default ">
	<div class="panel-heading">
		<h2 class="panel-title">
			Definicion de la idea
		</h2>
	</div>
	<div class="panel-body">
<div class="EntryUnit ">
	<div class="form-horizontal ">
	    <c:set var="btFieldError"><html:errors property="enu10"/></c:set>
		<c:if test="${not (empty btFieldError)}">
			<div class="alert alert-danger alert-dismissable">
				...
				<c:set var="btFieldError"><html:errors property="fld16"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld16" class="control-label col-md-2">Subject</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld16" styleClass="  form-control " property="fld16" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld16"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld14"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld14" class="control-label col-md-2">Description</label>
					<div class="controls col-md-10">
					<html:textarea styleId="fld14" styleClass="form-control " rows="8" property="fld14" readonly="false"/>
				...
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld11" class="control-label col-md-2">Title</label>
					<div class="controls col-md-10">
					<c:out value="${page15FormBean.map.fld11}" /><html:hidden property="fld11"/>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld12_locale"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld12_locale" class="control-label col-md-2">Date</label>
					<div class="controls col-md-10">
					<c:out value="${page15FormBean.map.fld12_locale}" /><html:hidden property="fld12_locale"/>
					</div>
				</div>
	</div>
</div>	
...
<div class="EntryUnit ">
		<div class="form-group form-btn">
					 <button title="Cancel" onclick="$('#page15FormBean')[0].target='_self'" class="btn  btn-default " id="button:ln108" name="button:ln108" type="submit" value="Cancel">
			Cancel</button>
					 <button title="Save and Work Later" onclick="$('#page15FormBean')[0].target='_self'" class="btn  btn-default " id="button:ln107" name="button:ln107" type="submit" value="Save and Work Later">
			Save and Work Later</button>
					 <c:set var="_wr_message"><bean:message key="Confirm Finish"/></c:set>
	<% pageContext.setAttribute("_wr_message", org.apache.commons.lang.StringEscapeUtils.escapeJavaScript((String) pageContext.getAttribute("_wr_message")));%>
			<button title="End" onclick=" var c = confirm('<c:out value="${_wr_message}"/>');  if (c) { $('#page15FormBean')[0].target='_self' }; return c; " class="btn  btn-default " id="button:ln106" name="button:ln106" type="submit" value="End">
			End</button>
		</div>
</div>	
				</div>
