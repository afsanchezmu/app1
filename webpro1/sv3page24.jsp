...
<c:if test="${not(empty sv3pdu5) and (sv3pdu5.dataSize gt 0)}">
    <div class="diagram-title" style="width:${sv3pdu5.width}px">
      <c:out value="${sv3pdu5.processName}"/>
	</div>
	<div class="plain ">
		<div class="ProcessDiagramUnit" style="position:relative; width:${sv3pdu5.width}px; height:${sv3pdu5.height}px; z-index:1">
          <c:if test="${not(empty sv3pdu5.data)}">
          <c:forEach var="current" varStatus="status" items="${sv3pdu5.data}">
			   ...
			   <c:choose>
                  <c:when test="${not(empty current.parentProcessInstanceOid)}">
                      <tr>
                        ...
                        <td nowrap="nowrap"><a class="diagram-link" href="<webratio:Link escapeXml="true" link="sv3pdu5parentLevel" position="index"/>" ><c:out value="${current.parentProcessInstanceName}"/></a></td>
                      </tr>
                  </c:when>
	              <c:when test="${not(empty current.parentProcesses)}">
	                  <c:forEach var="process" varStatus="status2" items="${current.parentProcesses}">
		                ...
		                <tr>
		                  <td nowrap="nowrap"><img src="Resources/parent_level.png"/></td>
		                  <td nowrap="nowrap"><a class="diagram-link" href="<webratio:Link escapeXml="true" link="sv3pdu5parentLevel" position="index,index2"/>" ><c:out value="${process.model.name}"/></a></td>
		                </tr>
		              </c:forEach>
	              </c:when> 
	           </c:choose>
			   <c:choose>
			         ...
			         <c:forEach var="oid" varStatus="status2" items="${current.subProcessInstanceOids}">
		               ...
		               <tr>
		                 <td nowrap="nowrap"><img src="Resources/sub_level.png"/></td>
		                  <td nowrap="nowrap"><a class="diagram-link" href="<webratio:Link escapeXml="true" link="sv3pdu5subLevel" position="index,index2"/>" ><c:out value="${current.subProcessInstanceNames[index2]}"/></a></td>
		               </tr>
		             </c:forEach>   
			      </c:when>
	              <c:when test="${not(empty current.subProcesses)}">
	                 <c:forEach var="process" varStatus="status2" items="${current.subProcesses}">
		               ...
		               <tr>
		                 <td nowrap="nowrap"><img src="Resources/sub_level.png"/></td>
		                 <td nowrap="nowrap"><a class="diagram-link" href="<webratio:Link escapeXml="true" link="sv3pdu5subLevel" position="index,index2"/>" ><c:out value="${process.model.name}"/></a></td>
		               </tr>
		             </c:forEach> 
	              </c:when> 
	           </c:choose>
          ...
          <img src="<webratio:BLOB value="${sv3pdu5.diagramFile}"/>" usemap="#diagramMap" width="${sv3pdu5.width}" height="${sv3pdu5.height}" border="0"/>
          <map name="diagramMap">
            <c:forEach var="current" varStatus="status" items="${sv3pdu5.data}">
			  <c:set var="index" value="${status.index}"/>
    ...
    <div class="diagram-bottom" style="width:${sv3pdu5.width}px"></div>
</c:if>
				</div>
					<div class="clear"></div>
</div>
