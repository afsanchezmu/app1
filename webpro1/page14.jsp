...
<script type="text/javascript">if (typeof wr !== "undefined") { wr.ui.html.resx.refreshLoaded(); }</script>
  ...
  <body>
	...
	<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
			<html:form action="form_page14.do${wrAjax ? '#!ajax=true' : ''}" styleId="page14FormBean" enctype="multipart/form-data">
		<c:if test="${wrBoxed}">
			<script>
				jQuery("#page14FormBean").prop("action", "${wrBaseURI}form_page14.do${wrAjax ? '#!ajax=true' : ''}");
			</script>
		</c:if>
			<html:hidden property="CSRFToken"/>
			<html:hidden property="lastURL" styleId="lastURL_page14"/>
...
<div class="panel panel-default ">
	<div class="panel-heading">
		<h2 class="panel-title">
			Please provide title and date
		</h2>
	</div>
	<div class="panel-body">
<div class="EntryUnit ">
	<div class="form-horizontal ">
	    <c:set var="btFieldError"><html:errors property="enu9"/></c:set>
		<c:if test="${not (empty btFieldError)}">
			<div class="alert alert-danger alert-dismissable">
				...
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld13" class="control-label col-md-2">title</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld13" styleClass="  form-control " property="fld13" readonly="false"/>
				...
				<c:set var="btFieldError"><html:errors property="fld15_locale"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld15_locale" class="control-label col-md-2">Date</label>
					<div class="controls col-md-10">
					<div class="input-calendar input-group">
				<html:text  styleId="fld15_locale" styleClass=" form-control " property="fld15_locale" readonly="false"/>
				<span class="input-group-btn">
				<c:choose><c:when test="false">
					<button disabled class="btn btn-default" id="fld15_locale_calendar" type="button"><span class="glyphicon glyphicon-calendar"></span></button>
				</c:when><c:otherwise>
					<button class="btn btn-default" id="fld15_locale_calendar" type="button"><span class="glyphicon glyphicon-calendar"></span>
							<script type="text/javascript">
								jQuery(function($) { 								
									var pattern = $.wr.calendar.dateConfigFromJava("${datePattern}");
									$('#fld15_locale').datepicker({
										showOn: "none",
										showWeek: true,
									...
									$('#fld15_locale_calendar').on('click', function(e) {
										e.preventDefault();
										$('#fld15_locale').datepicker('show');									
									});
								});
							</script>
					</button>
					...
					<c:set var="btFieldError"><html:errors property="fld15_locale"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
	</div>
</div>	
...
<div class="EntryUnit ">
	...
	<% pageContext.setAttribute("_wr_message", org.apache.commons.lang.StringEscapeUtils.escapeJavaScript((String) pageContext.getAttribute("_wr_message")));%>
			<button title="Next" onclick=" var c = confirm('<c:out value="${_wr_message}"/>');  if (c) { $('#page14FormBean')[0].target='_self' }; return c; " class="btn  btn-default " id="button:ln99" name="button:ln99" type="submit" value="Next">
			Next</button>
					 <button title="Save" onclick="$('#page14FormBean')[0].target='_self'" class="btn  btn-default " id="button:ln100" name="button:ln100" type="submit" value="Save">
			Save</button>
					 <button title="Cancel" onclick="$('#page14FormBean')[0].target='_self'" class="btn  btn-default " id="button:ln101" name="button:ln101" type="submit" value="Cancel">
			Cancel</button>
		</div>
</div>	
</div>
