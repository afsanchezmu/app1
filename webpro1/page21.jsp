...
<script type="text/javascript">if (typeof wr !== "undefined") { wr.ui.html.resx.refreshLoaded(); }</script>
  ...
  <body>
	...
	<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
		...
		<c:if test="${wrBoxed}">
			<script>
				jQuery("#page21FormBean").prop("action", "${wrBaseURI}form_page21.do${wrAjax ? '#!ajax=true' : ''}");
			</script>
		</c:if>
...
<c:if test="${not(empty pu29.exception)}">
<div class="frame">
	<div class="frame-title">An Error Occurred</div>
	<div class="frame-content">
<div class="plain ">
 <div class="plain ProcessUnit">
	<c:set var="exc" value="${pu29.exception}"/>
	<c:set var="errorTitle">
		<c:choose>
			<c:when test="${pu29.errorType eq 'ASSIGNED_ACTIVITY'}">Unable to start or resume the work item</c:when>
			<c:when test="${pu29.errorType eq 'COMPLETED_ACTIVITY'}">The work item has already been completed</c:when>
			<c:when test="${pu29.errorType eq 'ABORTED_ACTIVITY'}">The work item has been aborted</c:when>
	...
	<table>
		<c:choose>
		  <c:when test="${pu29.errorType eq 'ASSIGNED_ACTIVITY'}">
			<tr>
			  <td class="value">The work item "<b><c:out value="${pu29.activityInstanceName} [#${pu29.activityInstanceOid}]"/></b>" has already been assigned to another user and group.</td>
		    </tr>
		    <tr>
		       <td class="value" title="Assigned Group" style="padding-left:10px">
		       	Group: <b><c:out value="${pu29.activityGroupName} [#${pu29.activityGroupOid}]"/></b>
		       </td>
		    </tr>
		    <tr>
		       <td class="value" title="Assigned User" style="padding-left:10px">
		         User: <b><c:out value="${pu29.activityUserName} [#${pu29.activityUserOid}]"/></b>
		       </td>
		    </tr>
		    <tr>
		    	<td class="value">
		    		Go back to your <a class="link" href="<webratio:Link escapeXml="true" link="page17.link" position="index"/>">home</a> page and select another work item to execute.
		    	</td>
		    </tr>
		  </c:when>
		  <c:when test="${pu29.errorType eq 'COMPLETED_ACTIVITY'}">
			<tr>
			  <td class="value">The work item "<b><c:out value="${pu29.activityInstanceName} [#${pu29.activityInstanceOid}]"/></b>" has already been completed.</td>
		    </tr>
		    <tr><td class="value">Go back to your <a class="link" href="<webratio:Link escapeXml="true" link="page17.link" position="index"/>">home</a> page and select another work item to execute.</td></tr>
		  </c:when>
		  <c:when test="${pu29.errorType eq 'ABORTED_ACTIVITY'}">
			<tr><td class="value">The work item "<b><c:out value="${pu29.activityInstanceName} [#${pu29.activityInstanceOid}]"/></b>" has been aborted.</td></tr>
			<tr><td class="value">Go back to your <a class="link" href="<webratio:Link escapeXml="true" link="page17.link" position="index"/>">home</a> page and select another work item to execute.</td></tr>
		  </c:when>
		  <c:otherwise>
		    <tr>
